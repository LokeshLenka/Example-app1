    ...
    <div class="min-h-screen bg-blue-200 dark:bg-slate-950 flex justify-center items-start shadow-lg rounded-lg">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 mt-24 w-4/5">
            @foreach ($blogs as $blog)
                <div
                    class="w-full min-h-80 flex justify-center rounded-lg shadow-xl p-4 bg-blue-100 dark:bg-slate-900">
                    <div class="flex flex-col justify-between h-full w-full">
                        ...
                        <div class="flex justify-between">
                            <a href="{{ route('blog.show', $blog->id) }}"
                                class="text-indigo-500 dark:text-indigo-400 text-sm mt-4  " style="display:inline-block">
                                Profile
                            </a>

                            <!-- Read More Link -->
                            <a href="{{ route('blog.show', $blog->id) }}"
                                class="text-indigo-500 dark:text-indigo-400 text-sm mt-4 self-end  ">
                                Read More..
                            </a>
                        </div>
                    </div>
